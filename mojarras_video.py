...
from mojarras_client import MojarrasClient

client: MojarrasClient
...
class Area:
    ...
    def isAreaInsideOf(self, minimum: Point, maximum: Point) -> bool:
        if self.minimum.x >= minimum.x and self.minimum.y >= minimum.y and self.minimum.x <= maximum.x and self.minimum.y <= maximum.y:
            return True
        
        if self.maximum.x <= maximum.x and self.maximum.y <= maximum.y and self.maximum.x >= minimum.x and self.maximum.y >= minimum.y:
            return True

        return False
    
    def __str__(self) -> str:
        return f"(x: {self.minimum.x}, y: {self.minimum.y}) to (x: {self.maximum.x}, y: {self.maximum.y})"
...
class Drawer:
    ...
    def draw_rectangle(self, minimum: Point, maximum: Point):
        cv2.rectangle(self.frame, (minimum.x, minimum.y), (maximum.x, maximum.y), self.color,1)
...
def function_one():
    ...
    class GeometricCalculation:
        def __init__(self, minimumPoint: Point, maximumPoint: Point, frame) -> None:
            self.minimumPoint = minimumPoint
            self.maximumPoint = maximumPoint
            self.area = Area(self.minimumPoint, self.maximumPoint)
            self.drawer = Drawer(frame)
            self.drawer.draw_rectangle(self.minimumPoint, self.maximumPoint)
        ...
        def calculate_number_of_cars(self, areas: list) -> int:
            ...
            for idx, area in enumerate(areas):
                ...
                if isCarInside:
                    #print(f"current area: ${self.area} should be inside of: {area} and: {isCarInside}")
    ...
    while(video.isOpened()):
        ...
        for i in range(len(scores)):
            if ((scores[i] > .05) and (scores[i] <= 1.0)):
                ...
                area_calculator = GeometricCalculation(Point(xmin, ymin), Point(xmax, ymax), frame)
                car_areas.append(area_calculator)

        for car_area in car_areas:
            current_R1 = car_area.calculate_number_of_cars(areas_r1)
            current_R2 = car_area.calculate_number_of_cars(areas_r2)
            current_R3 = car_area.calculate_number_of_cars(areas_r3)
            current_R4 = car_area.calculate_number_of_cars(areas_r4)
            if current_R1 > R1:
                R1 = current_R1
            if current_R2 > R2:
                R2 = current_R2
        ...
        #Car counting labels
        cv2.putText(frame,f"R1: {R1}", (1700, 50), cv2.FONT_HERSHEY_DUPLEX, 2, (0,0,255), 2)
        cv2.putText(frame,f"R2: {R2}", (1700, 100), cv2.FONT_HERSHEY_DUPLEX, 2, (0,0,255), 2)
        cv2.putText(frame,f"R3: {R3}", (1700, 150), cv2.FONT_HERSHEY_DUPLEX, 2, (0,0,255), 2)
        cv2.putText(frame,f"R4: {R4}", (1700, 200), cv2.FONT_HERSHEY_DUPLEX, 2, (0,0,255), 2)
        
        car_areas.clear()
        
        global client
        print("This is running")
        client = MojarrasClient(R1, R2, R3, R4)
